...
opcode discr_dicstr, k, k[]
    kArr[] xin
    kIndx = 0
    kLen = kArr[kIndx]
    kRnd = random:k(0.0, 1.0)
    //kArrLen = lenarray(kArr)
    until  kRnd <= kLen do
        //printsk "kIndx=%d\n", kIndx
        kIndx = kIndx + 1
        kLen = kLen + kArr[kIndx]
        //printsk "kIndx=%d\tkLen=%f\tkRnd=%f\n", kIndx, kLen, kRnd
    enduntil
    xout kIndx
endop

opcode discr_dicstr, k, i[]
    iArr[] xin
    kIndx = 0
    kLen = iArr[kIndx]
    kRnd = random:k(0.0, 1.0)
    //kArrLen = lenarray(iArr)
    until  kRnd <= kLen do
        //printsk "kIndx=%d\n", kIndx
        kIndx = kIndx + 1
        kLen = kLen + iArr[kIndx]
        //printsk "kIndx=%d\tkLen=%f\tkRnd=%f\n", kIndx, kLen, kRnd
    enduntil
    xout kIndx
endop
