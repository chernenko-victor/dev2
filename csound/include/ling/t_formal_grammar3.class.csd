...
opcode get_index_of_nth_occurrence_of_number, k, k[]kk
	kArr[], kNumber, kOccurNum xin
	kIndexOccur init -1
	kIndex = 0
	kOccurCnt = 0
	
	fprintks 	$DUMP_FILE_NAME, "\nget_index_of_nth_occurrence_of_number :: kNumber = %d | kOccurNum = %d\n", kNumber, kOccurNum
	
	loopStart:
		if kArr[kIndex] == kNumber then
			kOccurCnt += 1
		endif
		if kOccurNum = kOccurCnt then
			kIndexOccur = kIndex
		endif
		kIndex += 1
		fprintks 	$DUMP_FILE_NAME, "\nget_index_of_nth_occurrence_of_number :: kNumber = %d | kOccurNum = %d | kArr[%d] = %d\n", kNumber, kOccurNum, kIndex,kArr[kIndex]  
		if kArr[kIndex] != 0 && kIndexOccur == -1 then 
			kgoto loopStart
		endif
...
opcode get_alter_rule_by_prob, k[], k[]k
	...
	fprintks 	$DUMP_FILE_NAME, "\nget_alter_rule_by_prob :: kAltRuleNumber = %d\n", kAltRuleNumber
	

	fprintks 	$DUMP_FILE_NAME, "\n ============ kIndxBegin = %d ==================\n", kIndxBegin
	if kAltRuleNumber > 0 then 
		kIndxBegin get_index_of_nth_occurrence_of_number kCurrentRule, $ALTERNATIVE_RULE_DELIM, kAltRuleNumber
		kIndxBegin += 1
	elseif kIndxBegin == -1 then
		kAltRuleNumber = 0
	endif
	fprintks 	$DUMP_FILE_NAME, "\n ============ kIndxEnd = %d ==================\n", kIndxEnd
	kIndxEnd get_index_of_nth_occurrence_of_number kCurrentRule, $ALTERNATIVE_RULE_DELIM, kAltRuleNumber+1
	fprintks 	$DUMP_FILE_NAME, "\nkIndxBegin = %d | kIndxEnd = %d\n", kIndxBegin, kIndxEnd
		
	;cut from kCurrentRule[kIndxBegin] to kCurrentRule[kIndxEnd] ->  kCurrentRuleAlter
	;kCurrentRuleAlter slicearray_my kCurrentRule, kIndxBegin, kIndxEnd ; Exeption
	;kArr[]  fillarray  1, 2, 3, 4, 5, 6, 7, 8, 9
	kCurrentRuleAlter slicearray_my kCurrentRule, kIndxBegin, kIndxEnd 
